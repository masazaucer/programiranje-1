...
let rec get k sez =
    match sez with
    | [] -> failwith "Ni vec seznama"
    | x :: xs ->
        if k <= 0 then x else get (k-1) xs
...
let rec insert nov k list =
   if k <= 0 then [nov] @ list else 
      match list with
      | [] -> [nov]
      | x :: xs -> x :: (insert nov (k-1) xs)

let rec insert nov k list = 
    match list with
    | [] -> [nov]
    | x :: xs -> if k <= 0 then nov :: sez else x :: (insert nov (k-1) xs)
